 ...
 * MACs</a> are available via {@link io.jsonwebtoken.Jws.alg Jws.alg}.</p>
 *
...
public interface SecureDigestAlgorithm<S extends Key, V extends Key>
     ...
     */
    interface Params<K extends Key, P extends Params<K, P>> extends DigestAlgorithm.Params<P>, Keyable<K, P> {
    }
     ...
     * @since JJWT_RELEASE_VERSION
     */
    interface VerifyParams<K extends Key, P extends VerifyParams<K, P>> extends Params<K, P>, DigestAlgorithm.VerifyParams<P> {
    }
     ...
     *
     * <p><code>
     * alg.digest(r -&gt; r.{@link Params#payload(Object) payload}(is).{@link Params#key(Key) key}(key));
     * </code></p>
     *
     ...
     *
     * @param c consumer supporting lambda-style specification of named digest {@link Params}.
     * @return the computed mac or signature for the request {@link SecureRequest#getPayload() payload}.
     * @since JJWT_RELEASE_VERSION
     */
    default byte[] digest(Consumer<Params<S, ?>> c) {
        Assert.notNull(c, "Consumer cannot be null");
        SecureDigestRequest.Builder<S> b = SecureDigestRequest.builder();
        c.accept(b);
     ...
     *
     * <p><code>
     * {@link #digest(Request) digest}(r -&gt; r.{@link Params#payload(Object) payload}(is).{@link Params#key(Key) key}(key));
     * </code></p>
     *
     * <p>Callers are expected to {@link InputStream#close() close} or {@link InputStream#reset() reset} the
     ...
     *
     * <p><code>
     * alg.verify(r -&gt; r.{@link VerifyParams#key(Key) key}(key).{@link VerifyParams#payload(Object) payload}(is).{@link VerifyParams#digest(byte[]) digest}(macOrSignature));
     * </code></p>
     *
     ...
     *
     * @param c consumer supporting lambda-style specification of named {@link VerifyParams params}.
     * @return {@code true} if a given mac or signature for an {@code InputStream} is authentic, {@code false} otherwise.
     * @since JJWT_RELEASE_VERSION
     */
    default boolean verify(Consumer<VerifyParams<V, ?>> c) {
        Assert.notNull(c, "Consumer cannot be null");
     ...
     *
     * <p><code>
     * {@link #verify(VerifyDigestRequest) verify}(r -&gt; r.{@link VerifyParams#key(Key) key}(key).{@link VerifyParams#payload(Object) payload}(is).{@link VerifyParams#digest(byte[]) digest}(macOrSignature));
     * </code></p>
     *
     * <p>Callers are expected to {@link InputStream#close() close} or {@link InputStream#reset() reset} the
     * {@code is} input stream if necessary after calling this method.</p>
