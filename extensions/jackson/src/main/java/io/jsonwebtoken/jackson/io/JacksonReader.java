...
public class JacksonReader<T> implements Reader<T> {
    ...
    public JacksonReader() {
        this(JacksonWriter.DEFAULT_OBJECT_MAPPER);
    }

    /**
     * Creates a new JacksonReader where the values of the claims can be parsed into given types. A common usage
     ...
     * your custom types and then use the {@link #JacksonReader(ObjectMapper)} constructor instead.
     *
     * @param claimTypeMap The claim name-to-class map used to deserialize claims into the given type
     */
    public JacksonReader(Map<String, Class<?>> claimTypeMap) {
    ...
    private JacksonReader(ObjectMapper objectMapper, Class<T> returnType) {
