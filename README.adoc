...
When you're ready to build and test the project locally, see link:CONTRIBUTING.md[CONTRIBUTING.md].
...
`Password.of(char[] passwordCharacters)`.
...
[,java]
----
Password key = Paswword.from(secretString.toCharArray());
----
...
. Use the `Jwk.builder()` method to create a `JwkBuilder` instance.
...
SecretJwk = Jwk.builder().key(key) // (1) and (2)
...
Jwk<?> jwk = Jwk.builder().add(jwkValues).build();
----
...
RsaPrivateJwk jwk = Jwk.builder().key(rsaPrivateKey)
...
RsaPrivateJwk rsaPrivJwk = Jwk.builder().rsaKeyPair(rsaKeyPair).build();

KeyPair ecKeyPair = getECKeyPair();
EcPrivateJwk ecPrivJwk = Jwk.builder().ecKeyPair(ecKeyPair).build();

KeyPair edEcKeyPair = getEdECKeyPair();
OctetPrivateJwk edEcPrivJwk = Jwk.builder().octetKeyPair(edEcKeyPair).build();
----
...
[,java]
----
RsaPrivateJwk privateJwk = Jwk.builder().key(rsaPrivateKey).build(); // or ecPrivateKey or edEcPrivateKey
...
[,java]
----
HashAlgorithm hashAlg = JwkThumbprint.alg.SHA256; // or SHA384, SHA512, etc.

Jwk<?> jwk = Jwk.builder(). /* ... */ .build();

JwkThumbprint sha256Thumbprint = jwk.thumbprint(); // SHA-256 thumbprint by default

JwkThumbprint anotherThumbprint = jwk.thumbprint(JwkThumbprint.alg.SHA512); // or a specified hash algorithm
----
...
* `jwkThumbprint.getHashAlgorithm()`: the specific `HashAlgorithm` used to compute the thumbprint. Many standard IANA
                                    hash algorithms are available as constants in the `JwkThumbprint.alg` utility class.
...
. Use the `JwkSet.builder()` method to create a `JwkSetBuilder` instance.
...
SecretJwk = JwkSet.builder()        // 1
    ...
    //.operationPolicy(Jwk.op      // 3, optional
    //     .policy()
    //     /* etc... */
    //     .build())
    //.provider(aJcaProvider)       //    optional
...
rejected. You can create a policy to override the default if desired using the `Jwk.op.policy()` builder method.
