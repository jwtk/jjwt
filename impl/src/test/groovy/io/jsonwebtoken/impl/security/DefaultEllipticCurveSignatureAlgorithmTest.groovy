...
class DefaultEllipticCurveSignatureAlgorithmTest {
    ...
    void testValidateSigningKeyWeakKey() {
        ...
        SignatureAlgorithms.values().findAll({ it.getId().startsWith('ES') }).each {
            try {
                it.sign(request)
    ...
    void testVerifyWithPrivateKey() {
        ...
        SignatureAlgorithms.values().findAll({ it instanceof DefaultEllipticCurveSignatureAlgorithm }).each {
    ...
    void invalidDERSignatureToJoseFormatTest() {
        def verify = { signature ->
            try {
                DefaultEllipticCurveSignatureAlgorithm.transcodeSignatureToConcat(signature, 132)
                fail()
...
//        signature[1] = 0x81
    ...
    void invalidECDSASignatureFormatTest() {
        try {
            def signature = new byte[257]
            Randoms.secureRandom().nextBytes(signature)
            DefaultEllipticCurveSignatureAlgorithm.transcodeSignatureToDER(signature)
            fail()
        } catch (JwtException e) {
            assertEquals e.message, 'Invalid ECDSA signature format'
        }
    ...
    void testPaddedSignatureToDER() {
        ...
        DefaultEllipticCurveSignatureAlgorithm.transcodeSignatureToDER(signature) //no exception
    }
    ...
    void verifySwarmTest() {
        SignatureAlgorithms.values().findAll({ it.getId().startsWith('ES') }).each { alg ->
            def withoutSignature = "eyJhbGciOiJFUzUxMiIsInR5cCI6IkpXVCJ9.eyJ0ZXN0IjoidGVzdCIsImlhdCI6MTQ2NzA2NTgyN30"
            def keypair = alg.generateKeyPair()
            def data = withoutSignature.getBytes("US-ASCII")
