...
class RsaPrivateJwkFactoryTest {
    ...
    void testNonCrtPrivateKey() { //tests a standard RSAPrivateKey (not a RSAPrivateCrtKey or RSAMultiPrimePrivateCrtKey):
        ...
        assertEquals Converters.BIGINT.applyTo(priv.getPrivateExponent()), jwk.get(DefaultRsaPrivateJwk.PRIVATE_EXPONENT.getId())
    }
    ...
    void testCreateJwkFromMinimalValues() { // no optional private values
        ...
        def jwkFromValues = Jwks.builder().putAll(map).build()
